...
def download_shard(r2, shard_key, on_stage=None):
    ...
    def _dl_chunk(i):
        ...
        with chunks_lock:
            chunks_done[0] += 1
            done_now = chunks_done[0]
        _stage("shard_chunk", shard_key=shard_key,
               done=done_now, total=n_workers)
...
def load_model(repo, device, label="model", force_download=False, revision=None,
    ...
    t0 = time.time()
    _stage("model_load_start", label=label, repo=repo, device=str(device),
           revision=(revision[:12] if revision else None))
    if not force_download:
        try:
            _stage("model_prefetch_start", label=label, repo=repo)
            ...
            _stage("model_prefetch_done", label=label, repo=repo,
    ...
    for attn_impl in ("flash_attention_2", "sdpa", "eager"):
        try:
            _stage("model_load_attn", label=label, attn=attn_impl, device=str(device))
    ...
    _stage("model_load_done", label=label, repo=repo,
           params_b=round(params, 2), elapsed_s=round(elapsed, 1))
    return model
...
def run_bootstrap_test(king_eval, challenger_eval, r2, shard_key, eval_n,
                       ...
                       n_bootstrap=10000, on_progress=None, on_stage=None):
    ...
    Calls on_stage(name, **extra) at the start/end of major sub-stages.
    """
    def _stage(name, **extra):
        if on_stage:
            try:
    ...
    data_offset, shard_data = download_shard(r2, shard_key, on_stage=on_stage)

    log.info("extracting %d sequences", actual_N)
    _stage("shard_extract", n=actual_N, shard_key=shard_key)
    ...
    _stage("bootstrap_start", n_batches=len(batches),
           batch_size=batch_size, total=actual_N)
    ...
    _stage("bootstrap_compute", n_bootstrap=n_bootstrap, n_samples=len(d))
